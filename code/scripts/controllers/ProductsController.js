...
export default class ProductsController extends WebcController {
  constructor(element, history) {
    ...
    this.onTagClick("transfer", (model, target, event) => {
      ...
      this.storageService.getRecord(
        constants.PRODUCTS_TABLE,
        gtin,
        (err, product) => {
          this.model.actionModalModel = {
            title: "Enter the company name to which the product is transferred",
            transferCode: $$.Buffer.from(JSON.stringify(product)).toString(
              "base64"
            ),
      ...
      this.showModalFromTemplate(
        ...
        (response) => {
          ...
          const product = JSON.parse(
            $$.Buffer.from(
              this.model.actionModalModel.transferCode,
              "base64"
            ).toString()
          );
      ...
      (err, prod) => {
        ...
        this.logService.log(
          ...
          (err) => {
            ...
            this.storageService.insertRecord(
              constants.PRODUCTS_TABLE,
              `${product.gtin}`,
              product,
              () => {}
