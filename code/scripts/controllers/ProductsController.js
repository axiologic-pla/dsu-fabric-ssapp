        ...
        this.storageService.getRecord(constants.PRODUCTS_TABLE, product.gtin, (err, prod) => {
            ...
            this.logService.log({
                ...
                this.storageService.safeBeginBatch(true, async err => {
                    ...
                    try {
                        await $$.promisify(this.storageService.insertRecord, this.storageService)(constants.PRODUCTS_TABLE, `${product.gtin}`, product);
                        this.model.prodDataSource.forceUpdate(true);
                        await this.storageService.commitBatchAsync();
                    } catch (e) {
                        const insertError = createOpenDSUErrorWrapper(`Failed to insert product ${product.gtin} into ${constants.PRODUCTS_TABLE}`, e);
                        try {
